...
def save_voice_settings(cid: str, settings: dict) -> None:
    ...
    # tts_provider lebte bisher nicht in ELEVENLABS_VOICE_SETTINGS_DEFAULT und wurde
    # vom `k in clean`-Filter oben verworfen — jeder Provider-Wechsel via
    # /api/tts_provider persistierte dadurch nie (nächster Server-Neustart/Reload
    # zeigte wieder "elevenlabs"). Explizit durchreichen, unabhängig vom Default-Dict.
...
def _tts_persist_and_schedule(cid: str, vid: str, text: str,
    ...
    """TTS-Pipeline-Entrypoint, aufgerufen von dashboard.py /api/voiceover_generate.

    Bis 2026-07-26 dispatchte diese Funktion zwischen ElevenLabs und MiniMax
    anhand von `tts_provider` in voice_settings.json (Phase 34). MiniMax wurde
    auf Nutzer-Wunsch vollständig entfernt (ungenutzt) -- ElevenLabs ist jetzt
    der einzige Provider. Name bleibt erhalten, da dashboard.py ihn aufruft."""
    if not vid:
